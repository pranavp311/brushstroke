...
export function noiseTerrainSceneElement(colors: ColorScheme, segments: number, interactive: boolean): string {
  return `
// Noise Terrain Background (Scene Element)
...
const bgMaterial = new THREE.ShaderMaterial({
  ...
  vertexShader: \`
    ...
    void main() {
      ...
      float n = snoise(pos.xz * 0.05 + uTime * 0.1 + scroll * 0.5) * 8.0;
      n += snoise(pos.xz * 0.1 + uTime * 0.05) * 4.0;
      n += snoise(pos.xz * 0.2) * 2.0;
      pos.y += n;
      vHeight = n;
...
window.updateBackgroundNoiseTerrain = updateBackground;`;
}
